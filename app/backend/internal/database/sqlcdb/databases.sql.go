...
const listDueDatabaseBackups = `-- name: ListDueDatabaseBackups :many
...
type ListDueDatabaseBackupsRow struct {
	ID             string         `json:"id"`
	BackupSchedule sql.NullString `json:"backup_schedule"`
}

func (q *Queries) ListDueDatabaseBackups(ctx context.Context) ([]ListDueDatabaseBackupsRow, error) {
	rows, err := q.db.QueryContext(ctx, listDueDatabaseBackups)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListDueDatabaseBackupsRow{}
	for rows.Next() {
		var i ListDueDatabaseBackupsRow
		if err := rows.Scan(&i.ID, &i.BackupSchedule); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
const setDatabaseBackupScheduleByIDAndUser = `-- name: SetDatabaseBackupScheduleByIDAndUser :exec
UPDATE database_services
SET backup_schedule = $1, next_backup_at = $2, updated_at = $3
WHERE id = $4 AND user_id = $5
`

type SetDatabaseBackupScheduleByIDAndUserParams struct {
	BackupSchedule sql.NullString `json:"backup_schedule"`
	NextBackupAt   sql.NullTime   `json:"next_backup_at"`
	UpdatedAt      sql.NullTime   `json:"updated_at"`
	ID             string         `json:"id"`
...
func (q *Queries) SetDatabaseBackupScheduleByIDAndUser(ctx context.Context, arg SetDatabaseBackupScheduleByIDAndUserParams) error {
	_, err := q.db.ExecContext(ctx, setDatabaseBackupScheduleByIDAndUser,
		arg.BackupSchedule,
		arg.NextBackupAt,
		arg.UpdatedAt,
		arg.ID,
...
const setDatabaseNextBackupAt = `-- name: SetDatabaseNextBackupAt :exec
UPDATE database_services
SET next_backup_at = $1
WHERE id = $2
`

type SetDatabaseNextBackupAtParams struct {
	NextBackupAt sql.NullTime `json:"next_backup_at"`
	ID           string       `json:"id"`
}
