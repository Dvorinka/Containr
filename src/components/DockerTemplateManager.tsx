...
import { CheckCircle, Circle, AlertCircle, Play, Square, Trash2, Download, ExternalLink } from 'lucide-react';
...
const DockerTemplateManager: React.FC = () => {
  const [templates, setTemplates] = useState<Template[]>([
    ...
    },
    {
      id: 'vaultwarden',
      name: 'Vaultwarden',
      description: 'Password manager (Bitwarden alternative)',
      category: 'Security',
      difficulty: 'Medium',
      port: 8080,
      icon: '🛡️',
    ...
    },
    {
      id: 'cloudreve',
      name: 'Cloudreve',
      description: 'File manager with multiple backends',
      category: 'Storage',
      difficulty: 'Medium',
      port: 5212,
      icon: '📁',
  ...
  const deployAll = async () => {
    ...
    for (const template of easyTemplates) {
      if (!template.deployed) {
        await deployTemplate(template);
        await new Promise(resolve => setTimeout(resolve, 2000));
      }
    }
  };
  ...
  const runningCount = templates.filter(t => t.status === 'running').length;
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button onClick={deployAll} variant="outline">
            Deploy All Easy Templates
          </Button>
        </div>
      </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {templates.length > 0 ? Math.round((deployedCount / templates.length) * 100) : 0}%
                </p>
              </div>
            </div>
          </CardContent>
