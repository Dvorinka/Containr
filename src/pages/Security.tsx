...
import { Shield, AlertTriangle, RefreshCw, Lock, Eye, CheckCircle } from 'lucide-react';
...
export default function SecurityPage() {
  ...
  const metricsQuery = useQuery({
    ...
    enabled: !!projectId && isFeatureEnabled('security'),
  });
  ...
  const startScanMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: 'Comprehensive security scan has been initiated',
      });
    },
    onError: (err: Error) => {
      toast({
        title: 'Error',
        description: err.message || 'Failed to start security scan',
        variant: 'destructive',
      });
    },
  });
  ...
  if (!isFeatureEnabled('security')) {
    return (
      <div className="p-6 text-center">
        ...
        <h2 className="text-xl font-semibold mb-2">Security Features Not Available</h2>
        <p className="text-muted-foreground">Upgrade to production mode to access security features</p>
      </div>
    );
  }
    ...
    <div className="p-4 md:p-6 lg:p-8 space-y-8">
      ...
      <Card>
        ...
        <CardContent className="flex flex-col md:flex-row gap-4 items-start md:items-end">
          ...
          >
            <RefreshCw className={`w-4 h-4 mr-2 ${startScanMutation.isPending ? 'animate-spin' : ''}`} />
        ...
        <CardContent className="space-y-3">
          {vulnerabilitiesQuery.isLoading && (
            <div className="flex items-center justify-center py-4">
              ...
              <span className="text-sm text-muted-foreground">Loading vulnerabilities...</span>
            </div>
          )}
